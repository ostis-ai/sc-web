...
scgLayout.prototype={_initScgLayout:function(a){this.config=a;this.config=this._defaultConfig;this._renderer=this._graph=null;this.nodeForce={}},_defaultConfig:{MAX_REPULSIVE_LENGTH:350,MAX_REPULSIVE_SQUARED:122500,AVG_ATTRACTIVE_LENGTH:100,DEF_START_BOX_HEIGHT:101,MAX_ITERATIONS_CYCLE:21,REPULSIVE_PARAMETER:12E4,ATTRACTIVE_PARAMETER:2,TIME_VELOCITY:1,SCREEN_CENTER_X:200,SCREEN_CENTER_Y:250,MAX_FORCE:15,NUM_STEP_ITERATION:5,MIN_MOVEMENT:2},setGraph:function(a){this._graph=a},setRenderer:function(a){this._renderer=
a},layout:function(){var a=this;setTimeout(function(){var b=a._layoutStep(a.config.NUM_STEP_ITERATION);a._renderer.reDraw();a._renderer._layerNodes.draw();b&&a.layout()},100)},_layoutStep:function(a){for(var b=0;b<a;b++){for(var c in this._graph.nodes){this.setNodeForce(this._graph.nodes[c],0,0);for(var e in this._graph.nodes){if(e==c)break;var d=this.getRelation(this._graph.nodes[c],this._graph.nodes[e]);console.log(this._graph.nodes[e].id+" Repulsive Pairs deltaX "+d.deltaX+"| deltaY "+d.deltaY+
"| sqDistance "+d.sqDistance);if(d.sqDistance>this.config.MAX_REPULSIVE_SQUARED)break;if(0>=d.sqDistance)break;this.addNodeForce(this._graph.nodes[c],d.deltaX,d.deltaY,d.sqDistance,this.getRepulsiveForce(d.sqDistance))}for(e in this._graph.nodes[c]._incidentArcs){d=this._graph.nodes[c]._incidentArcs[e]._endNode;d==this._graph.nodes[c]&&(d=this._graph.nodes[c]._incidentArcs[e]._beginNode);if(d==this._graph.nodes[c])break;d=this.getRelation(this._graph.nodes[c],d);if(0>=d.sqDistance)break;this.addNodeForce(this._graph.nodes[c],
d.deltaX,d.deltaY,d.sqDistance,this.getAttractiveForce(d.sqDistance))}}var f=d=0;for(c in this._graph.nodes){posX=this._graph.nodes[c].getPosition().x;posY=this._graph.nodes[c].getPosition().y;var g=this.getNodeForce(this._graph.nodes[c]).x,h=this.getNodeForce(this._graph.nodes[c]).y;console.log(this._graph.nodes[c].id+" current forceX "+g+"forceY "+h);g>this.config.MAX_FORCE&&(g=this.config.MAX_FORCE);g<-this.config.MAX_FORCE&&(g=-this.config.MAX_FORCE);h>this.config.MAX_FORCE&&(h=this.config.MAX_FORCE);
h<-this.config.MAX_FORCE&&(h=-this.config.MAX_FORCE);g*=this.config.TIME_VELOCITY;h*=this.config.TIME_VELOCITY;d=Math.max(d,Math.abs(g));f=Math.max(f,Math.abs(h));this._graph.nodes[c].setPosition(posX+g,posY+h)}if(d<this.config.MIN_MOVEMENT&&f<this.config.MIN_MOVEMENT)return!1}return!0},getRelation:function(a,b){var c=b.getPosition().x-a.getPosition().x,e=b.getPosition().y-a.getPosition().y,d=[];d.deltaX=c;d.deltaY=e;d.sqDistance=c*c+e*e;return d},getArcRelation:function(a,b){var c=b.getPosition().x-
a.getPosition().x,e=b.getPosition().y-a.getPosition().y,d=[];d.deltaX=c;d.deltaY=e;d.sqDistance=c*c+e*e;return d},getRepulsiveForce:function(a){return-this.config.REPULSIVE_PARAMETER/a},getAttractiveForce:function(a){return-this.config.ATTRACTIVE_PARAMETER*(this.config.AVG_ATTRACTIVE_LENGTH-Math.sqrt(a))},setNodeForce:function(a,b,c){this.nodeForce[a.id]={x:b,y:c}},addNodeForce:function(a,b,c,e,d){e=d*d/e;var f=this.nodeForce[a.id];f||(this.nodeForce[a.id]={x:0,y:0},f=this.nodeForce[a.id]);f.x+=b*
e*(d%1);f.y+=c*e*(d%1)},getNodeForce:function(a){return this.nodeForce[a.id]}};var scgViewerWindow=function(a){this._initWindow(a)};
scgViewerWindow.prototype={_initWindow:function(a){this.domContainer=a.container;this._currentLanguage=this.renderer=null},receiveData:function(a){if(a=this._buildGraph(a)){$("#"+this.domContainer).empty();this.renderer=new scgKineticRenderer({graph:a,container:this.domContainer,width:800,height:600});var b=new scgLayout({});b.setGraph(a);b.setRenderer(this.renderer);b.layout();this.renderer.render()}else console.warn("Build graph error")},destroy:function(){return!0},_buildGraph:function(a){return GraphBuilder.buildGraph(a)},
translateIdentifiers:function(a){var b=this._getObjectsForTranslate(),c=this;SCWeb.core.Translation.translate(b,a,function(a){c._translateObjects(a)})},getIdentifiersLanguage:function(){return this._currentLanguage},_getObjectsForTranslate:function(){var a=this.renderer.getGraph().nodes,b=[],c;for(c in a)b.push(c);return b},_translateObjects:function(a){var b=this.renderer.getGraph(),c;for(c in a){var e=b.getNodeById(c);e&&(e.idf=a[c])}this.renderer.reDraw();this.renderer._layerNodes.draw()}};
SCgComponent={type:0,outputLang:"format_scg_json",factory:function(a){return new scgViewerWindow(a)},addArgument:function(a){SCWeb.core.Arguments.appendArgument(a)}};$(document).ready(function(){SCWeb.core.ComponentManager.appendComponentInitialize(function(){SCWeb.core.ComponentManager.registerComponent(SCgComponent)})});var scArc=function(a,b,c){this._initScArc(a,b,c)};
...
e),b.addNode(g),e++,d[f.id]=g)}this._buildArcs(c,d,b);return b},_buildNode:function(a){return new scNode({id:a.id,idf:a.id,type:a.el_type})},_initNodePosition:function(a,b){var c=this._calcXNodePos(b),e=this._calcYNodePos(b);a.setPosition(c,e)},_calcXNodePos:function(a){return 100+25*a},_calcYNodePos:function(a){return 70+30*a},_buildLinkNode:function(a){return new scNode({id:a.id,idf:a.id+this.LINK_SUFFIX,type:this.nodeType.LINK})},_buildArcs:function(a,b,c){for(var e=!0;0<a.length&&e;)for(idx in e=
!1,a){var d=a[idx],f=d.begin,g=d.end;b.hasOwnProperty(f)&&b.hasOwnProperty(g)&&(f=this._buildArc(d,b[f],b[g]),e=!0,a.splice(idx,1),b[d.id]=f,c.addArc(f))}},_buildArc:function(a,b,c){return new scArc(b,c,{id:a.id,type:a.el_type})}},scNode=function(a){this._initScNode(a)};
...
return this._arrow},__defaultArrowConfig:{strokeWidth:1,stroke:"black",fill:"black",lineCap:"butt",points:[-3,-2,3,-2,0,6],rotate:0},_defaultBreakConfig:{x:100,y:100,radius:5,stroke:"#666",fill:"#ddd",strokeWidth:1,draggable:!0},__defaultLineConfig:{strokeWidth:2,stroke:"black",lineCap:"butt",points:[0,0,1,1],lineJoin:"round",detectionType:"pixel"}};
...
scgNode.prototype={_initNode:function(a,b,c){this.id=0;this.layer=a;this.node=b;this.idfDrawObj=null;this.color="#000";this._renderer=c;this.draw();this.reDraw()},getRenderer:function(){return this._renderer},setColor:function(a){this.color=a},draw:function(){this.layer.add(this._getIdf());this._draw();return this},reDraw:function(){var a=this.node.getPosition();this._getIdf().setPosition(a.x+10,a.y+10);this._reDraw();return this},_getIdf:function(){if(null!=this.idfDrawObj)return this.idfDrawObj.setText(this.node.idf),
this.idfDrawObj;this._defaultIdfConfig.text=this.node.idf;return this.idfDrawObj=new Kinetic.Text(this._defaultIdfConfig)},_defaultIdfConfig:{x:110,y:110,text:"",textFill:"gray"}};var scgNodeConst=function(a,b,c){this._initNodeConst(a,b,c)};
...
function(){a.node.setPosition(this.getPosition());a.getRenderer().reDraw()});this._circle.on("mouseover",function(){a.setColor("#f0f");a.getRenderer().reDraw()});this._circle.on("mouseout",function(){a.setColor("#000");a.getRenderer().reDraw()});this._circle.on("click",function(){SCWeb.core.utils.Keyboard.ctrl&&SCgComponent.addArgument(a.node.id)})}this._circle.setStroke(this.color);return this._circle},_getBg:function(){if(null==this._bg){this._bg=new Kinetic.Shape;var a=this;this._bg.setDrawFunc(function(b){a._bgDrawFunc.call(this,
b,a)})}return this._bg},_bgDrawFunc:function(a,b){var c=scgNodeConst.drawFunc[b.node.getStructType()];a.strokeStyle=b.color;a.fillStyle=b.color;c.call(this,a)},_defaultCircleConfig:{x:100,y:100,radius:9,stroke:"#000",strokeWidth:4,draggable:!0}};Kinetic.Global.extend(scgNodeConst,scgNode);
...
function(){a.node.setPosition(this.getPosition());a.getRenderer().reDraw()});this._rect.on("mouseover",function(){a.node.isActive=!0});this._rect.on("mouseout",function(){a.node.isActive=!1});this._rect.on("click",function(){SCWeb.core.utils.Keyboard.ctrl&&SCgComponent.addArgument(a.node.id)})}this._rect.setStroke(this.color);return this._rect},_getBg:function(){if(null==this._bg){this._bg=new Kinetic.Shape;var a=this;this._bg.setDrawFunc(function(b){a._bgDrawFunc.call(this,b,a)})}return this._bg},
_bgDrawFunc:function(a,b){var c=scgNodeVar.drawFunc[b.node.getStructType()];a.strokeStyle=b.color;a.fillStyle=b.color;c.call(this,a)},_defaultRectConfig:{x:100,y:100,width:14,height:14,offset:{x:7,y:7},stroke:"#000",strokeWidth:4,draggable:!0}};
